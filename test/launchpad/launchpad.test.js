// npx hardhat test test/launchpad/launchpad.test.js
...
describe("Launchpad", function () {
  ...
  // NFT contract data
  const nftDescription = "AlwaysLiquid is a new unique NFT launchpad";
  const nftImage = "https://alwaysliquid.com/assets/img/preview.jpg";
  const nftMetadataName = "AlwaysLiquid NFT";
  const nftName = "AlwaysLiquid NFT";
  const nftSymbol = "ALNFT";
  ...
  it("creates a new NFT contract via launchpad and mints&burns a bunch of NFTs", async function () {
    ...
    const AlwaysLiquidNft = await ethers.getContractFactory("AlwaysLiquidNft");
    const nftContract = await AlwaysLiquidNft.attach(nftContractAddress);
    ...
    await expect(nftContract.connect(user1).burn(2)).to.be.revertedWith("AlwaysLiquidNft: caller is not owner nor approved");
    ...
    await expect(nftContract.connect(user1).setMintingFeePercentage(ethers.utils.parseEther("0.06"))).to.be.revertedWith("AlwaysLiquidNft: fee must be lower than 5%");
