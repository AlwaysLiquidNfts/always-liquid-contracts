...
/// @title Post NFT stats contract
/// @author Tempe Techie
/// @notice Contract that keeps track of who minted which Post NFT IDs
contract PostStats is OwnableWithManagers {
  ...
  function addMintedPostId(address _user, uint256 _postId) external {
    require(_msgSender() == minterAddress, "PostStats: Only minter can add minted post ID");
    getMintedPostIds[_user].push(_postId);
    postMintedCounter[_postId] += 1;
  }

  function addMintedWei(address _user, uint256 _wei) external {
    require(_msgSender() == minterAddress, "PostStats: Only minter can add minted wei");
    getMintedWei[_user] += _wei;
  }

  function addWeiEarnedByAuthorPerPostId(uint256 _postId, uint256 _wei) external {
    require(_msgSender() == minterAddress, "PostStats: Only minter can add wei earned by author per post ID");
