...
/// @title Post NFT
/// @author Tempe Techie
contract PostNft is ERC1155, OwnableWithManagers, ReentrancyGuard {
  ...
  function uri(uint256 _tokenId) public view override returns (string memory) {
    require(_tokenId < counter, "PostNft: Token id does not exist");

    Post memory post = getPost[_tokenId];

    return IPostNftMetadata(metadataAddress).getMetadata(
      _tokenId, 
      post.postId, 
      post.author, 
      post.textPreview, 
  ...
  function mint(
    ...
    require(_msgSender() == minterAddress, "PostNft: Only minter can mint");
    require(bytes(_textPreview).length <= maxTextPreviewLength, "PostNft: Text preview is too long");
    ...
    if (mintTime != 0) {
      require(block.timestamp <= (getPost[tokenId].timestamp + mintTime), "PostNft: Minting deadline has passed");
    }
  ...
  function ownerChangeImage(uint256 _tokenId, string memory _newImage) external onlyManagerOrOwner {
    require(_tokenId < counter, "PostNft: Token id does not exist");
    require(!postChangingDisabledForever, "PostNft: Post changing is disabled forever");
  ...
  function ownerChangeTextPreview(uint256 _tokenId, string memory _newTextPreview) external onlyManagerOrOwner {
    require(_tokenId < counter, "PostNft: Token id does not exist");
    require(!postChangingDisabledForever, "PostNft: Post changing is disabled forever");
    require(bytes(_newTextPreview).length <= maxTextPreviewLength, "PostNft: Text preview is too long");
