...
@author Tempe Techie
*/
contract AlwaysLiquidNft is ERC721, ERC721Enumerable, OwnableWithManagers, ReentrancyGuard {
  ...
  */
  ...
  function burn(uint256 tokenId) external nonReentrant returns (uint256) {
    ...
    require(_isApprovedOrOwner(msg.sender, tokenId), "AlwaysLiquidNft: caller is not owner nor approved");
    ...
    require(successMfReceiver, "AlwaysLiquidNft: Failed to send protocol fee");

    // send payment to the burn caller
    (bool successMsgSender, ) = msg.sender.call{value: price - ownerFee - protocolFee}("");
    require(successMsgSender, "AlwaysLiquidNft: Failed to send payment");
  ...
  function setMintingFeePercentage(uint256 mintingFeePercentage_) external onlyManagerOrOwner {
    require(mintingFeePercentage_ < (5 * 1e16), "AlwaysLiquidNft: fee must be lower than 5%");
    mintingFeePercentage = mintingFeePercentage_;
  }
  ...
  function setMintingFeeReceiver(address mintingFeeReceiver_) external {
    require(msg.sender == IFactory(factoryAddress).owner(), "AlwaysLiquidNft: Only factory owner can set a new mintingFeeReceiver");
    mintingFeeReceiver = mintingFeeReceiver_;
  }
